    ...
    async def api_call(self, method, args=None, params=None) -> dict | list:
        ...
        try:
            result = await self.session.post(
                "http://www.deezer.com/ajax/gw-light.php", params=p, timeout=30, json=args, headers=self.http_headers
            )
        ...
        if len(result_json["error"]):
            if result_json["error"] == {"GATEWAY_ERROR": "invalid api token"} or result_json["error"] == {
    ...
    async def api_call(self, method, args=None):
        ...
        try:
            result = await self.session.get(
                "https://api.deezer.com/" + method, params=args, headers=self.http_headers, timeout=30
            )
            result_json = result.json()
        except (ConnectError, TimeoutException):
            await asyncio.sleep(2)
...
class AsyncDeezer(Deezer):
    ...
    async def get_track_url(self, track_token, track_format):
        tracks = await self.get_tracks_url(
            [
                track_token,
            ],
            track_format,
        ...
        try:
            request = await self.session.post(
                ...
                json={
                    ...
                    "media": [{"type": "FULL", "formats": [{"cipher": "BF_CBC_STRIPE", "format": track_format}]}],
    ...
    async def _post_login(self, user_data):
        self.childs = []
        family = (
            user_data["USER"]["MULTI_ACCOUNT"]["ENABLED"] and not user_data["USER"]["MULTI_ACCOUNT"]["IS_SUB_ACCOUNT"]
        )
        if family:
